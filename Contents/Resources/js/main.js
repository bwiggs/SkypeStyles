...
function cocoaDcWrapper()
{
	var args = $A(arguments), f = args.shift();
	if ( window['dC'] )
	{
		return dC[f].apply(dC, args);
	}
	else
	{
		console.log('dC.' + f + '(): no cocoa dC available');
	}
	return null;
}
...
SCS.ErrorHandler = function() {
	...
	/**
	 * Get the conversation script
	 * 
	 * @return The conversation script
	 * @type String
	 */
	this.getConversationScript = function()
	{
		return _all.join("\n");
	};
	 ...
	 * @param {String} fname Name of the function to return as part of the string
	 * @param {Object} context A context to give to the event
	 * @return Status string or false if unknown code
	 * @type {String}
	 */
	this.showError = function(code, fname, context) {
		var errString = "";
		var line, args;
		if (typeof code != "undefined") {
			...
			if ( window['debug'] && window.debug == true) {
				...
				if ( code == 200 && fname.charAt(0) != '_' && context )
				{
					line = 'SCS.conv.' + fname + '(';
					args = [];
			...
			if ( code == 200 && fname && window['SkSt'] && SkSt.ready )
			{
				context.eventName = fname;
				document.fire('scs:' + fname, context || null);
				document.fire('scs:event', context || null);
			}
			return errString;
		} else {
			return false;
...
SCS.Conversation = function() {
		...
		if (_container.length > 0) {
			var _layoutCheck = function() {
				...
				if (w < 540) {
					...
					if (_mode.name != "compact") {
						...
						_mode.left = 23;
						_mode.right = 23;
						cocoaDcWrapper('setMode_leftMargin_rightMargin_', _mode.name, _mode.left, _mode.right);
					}
				} else {
					_container.removeClass("Òcompact");
			...
			$(window).scroll(function() {
				if (window.scrollY < 11 && _scrolltimer == false) {
					cocoaDcWrapper('scrollWindowToTopEvent');
					_scrolltimer = true;
				}
	...
	this._item = function() {
		if (_container.length > 0) {
			$(".item .head .sender a:not(.profile)").live("click", function(e) {
				...
				e.preventDefault();
				cocoaDcWrapper('openConversationWithIdentity_', $a.attr("href"));
			});
			$(".item .head .sender img, .item .head .sender span.profile").live("click", function(e) {
				...
				if ($i.attr("data-sender")) {
					cocoaDcWrapper('showProfileOfIdentity_', $i.attr("data-sender"));
				}
			});
			$(".item button").live("click", function(e) {
				...
				if ($b.hasClass("actions")) {
					cocoaDcWrapper('showContextMenuForID_', $i.attr("id"));
				}
			});
	...
	this._history = function() {
		if (_container.length > 0) {
			$(".item.history a").live("click", function(e) {
				...
				e.preventDefault();
				cocoaDcWrapper('loadHistoryInDays_', $a.attr("href"));
			});
			...
			$(".item.voicemail button").live("click", function() {
				...
				if ($b.hasClass("control")) {
					...
					if ($(this).hasClass("stop")) {
						if (cocoaDcWrapper('stopVoicemailWithID_', $vm.attr("id"))) {
							self.voicemailStop($vm.attr("id"));
						}
					} else {
						if (cocoaDcWrapper('playVoicemailWithID_', $vm.attr("id"))) {
				...
				if ($b.hasClass("delete")) {
					if (cocoaDcWrapper('stopVoicemailWithID_', $vm.attr("id"))) {
						if (cocoaDcWrapper('deleteVoicemailWithID_', $vm.attr("id"))) {
	...
	this._message = function() {
		if (_container.length > 0) {
			$(".item.message button").live("click", function(e) {
				...
				// Edit message
				if ($b.hasClass("edit")) {
					cocoaDcWrapper('startMessageEditWithID_', $m.attr("id"));
				}
				
				// Delete message
				if ($b.hasClass("delete")) {
					cocoaDcWrapper('removeMessageWithID_', $m.attr("id"));
				}
			});
			$(".item.message.editable").live("mouseover", function() {
				...
				if (!cocoaDcWrapper('isMessageEditable_', $m.attr("id"))) {
			...
			$(".item.message.editable .body").live("dblclick", function(e) {
				...
				if (cocoaDcWrapper('isMessageEditableInline_', $m.attr("id"))) {
					if (cocoaDcWrapper('startMessageInlineEditWithID_body_', $m.attr("id"), $(".body", $m).html())) {
			...
			$(window).bind("keyup", function(e) {
				if (e.which == 27) {
					$(".item .editMode").each(function() {
						var $m = $(this).closest(".item");
						if (cocoaDcWrapper('cancelMessageInlineEditWithID_', $m.attr("id"))) {
			...
			$(".item.message.editable .editMode").live("keydown", function(e) {
				...
				if (e.which == 13) {
					if (!e.shiftKey) {
						e.preventDefault();
						if (cocoaDcWrapper('stopMessageInlineEditAndSaveWithID_body_', $m.attr("id"), $mb.html())) {
							self.messageEndInlineEdit($m.attr("id"));
						}
					}
				}
	...
	this._transfer = function() {
		if (_container.length > 0) {
			...
			$(".item.transfer .icon").live("dblclick", function() {
				...
				cocoaDcWrapper('doubleClickIconWithID_index_', $ft.attr("id"), $(".instance", $ft).index($ins));
			});
			...
			$(".item.transfer button").live("click", function() {
				...
				if ($b.hasClass("accept")) {
					if (cocoaDcWrapper('acceptTransferWithID_index_', $ft.attr("id"), $(".instance", $ft).index($ins))) {
				...
				if ($b.hasClass("decline")) {
					if (cocoaDcWrapper('cancelTransferWithID_index_', $ft.attr("id"), $(".instance", $ft).index($ins))) {
				...
				// Delete transfers
				if ($b.hasClass("delete")) {
					if (cocoaDcWrapper('deleteTransferWithID_index_', $ft.attr("id"), $(".instance", $ft).index($ins))) {
				...
				if ($b.hasClass("quicklook")) {
					cocoaDcWrapper('showTransferInQuickLookWithID_index_', $ft.attr("id"), $(".instance", $ft).index($ins));
				}
				
				// Reveal transfer in Finder
				if ($b.hasClass("reveal")) {
					cocoaDcWrapper('revealTransferInFinderWithID_index_', $ft.attr("id"), $(".instance", $ft).index($ins));
				}
			});
	...
	this.appendItem = function(html, scroll) {
		if (_container.length > 0) {
			...
			return SCS.err.showError(200, "appendItem", arguments);
		} else {
	...
	this.scrollToItem = function(id) {
		...
		if (_container.length > 0) {
			...
			if ($i.length > 0) {
				...
				return SCS.err.showError(200, "scrollToItem", arguments);
			} else {
	...
	this.prependItem = function(html, scroll) {
		if (_container.length > 0) {
			...
			return SCS.err.showError(200, "prependItem", arguments);
		} else {
	...
	this.appendBulk = function(html, scroll, origin, location) {
		if (_container.length > 0) {
			...
			return SCS.err.showError(200, "appendBulk", arguments);
		} else {
	...
	this.removeItem = function(id, scroll) {
		...
		if (_container.length > 0) {
			...
			if ($i.length > 0) {
				...
				return SCS.err.showError(200, "removeItem", arguments);
			} else {
				return SCS.err.showError(511, "scrollToItem");
			}
		} else {
	...
	this.typingUpdate = function(html, status, scroll) {
		if (_container.length > 0) {
			...
			return SCS.err.showError(200, "typingUpdate", arguments);
		} else {
	...
	this.scrollToEnd = function() {
		if (_container.length > 0) {
			...
			return SCS.err.showError(200, "scrollToEnd", arguments);
		} else {
		...
		if (_container.length > 0) {
			...
			if ($ft.length > 0) {
				if (typeof index != "undefined" && typeof index == "number" && index >= 0) {
					...
					return SCS.err.showError(200, "transferAccept", arguments);
				} else {
		...
		if (_container.length > 0) {
			...
			if ($ft.length > 0) {
				if (typeof index != "undefined" && typeof index == "number" && index >= 0) {
					...
					return SCS.err.showError(200, "transferCancel", arguments);
				} else {
		...
		if (_container.length > 0) {
			...
			if ($ft.length > 0) {
				if (typeof index != "undefined" && typeof index == "number" && index >= 0) {
					...
					return SCS.err.showError(200, "transferReady", arguments);
				} else {
	...
	this.transferPosition = function(id, index, percentage, text, connecting) {
		...
		if (_container.length > 0) {
			if ($ft.length > 0 && $(".progress:eq("+index+")", $ft).length > 0 && $(".size:eq("+index+")", $ft).length > 0) {
				...
				return SCS.err.showError(200, "transferPosition", arguments);
			} else {
		...
		if (_container.length > 0) {
			...
			if ($ft.length > 0) {
				if (typeof index != "undefined" && typeof index == "number" && index >= 0) {
					...
					return SCS.err.showError(200, "transferComplete", arguments);
				} else {
	...
	this.transferUpdateHead = function(id, index, status, text) {
		...
		if (_container.length > 0) {
			if ($ft.length > 0 && $(".time:eq("+index+")", $ft).length > 0) {
				...
				return SCS.err.showError(200, "transferUpdateHead", arguments);
			} else {
	...
	this.transferUpdateStatus = function(id, index, status) {
		...
		if (_container.length > 0) {
			if ($ft.length > 0 && $(".instance:eq("+index+")", $ft).length > 0) {
				...
				return SCS.err.showError(200, "transferUpdateStatus", arguments);
			} else {
	...
	this.voicemailPlay = function(id) {
		...
		if (_container.length > 0) {
			if ($vm.length > 0) {
				...
				return SCS.err.showError(200, "voicemailPlay", arguments);
			} else {
	...
	this.voicemailStop = function(id) {
		...
		if (_container.length > 0) {
			if ($vm.length > 0) {
				...
				return SCS.err.showError(200, "voicemailStop", arguments);
			} else {
	...
	this.voicemailPosition = function(id, percentage, time) {
		...
		if (_container.length > 0) {
			if ($vm.length > 0 && $(".progress", $vm).length > 0 && $(".played", $vm).length > 0) {
				...
				$(".played", $vm).html(time);
				return SCS.err.showError(200, "voicemailPosition", arguments);
			} else {
	...
	this.smsUpdateStatus = function(id, index, status, text) {
		...
		if (_container.length > 0) {
			if ($s.length > 0 && $(".time:eq("+index+")", $s).length > 0) {
				...
				return SCS.err.showError(200, "smsUpdateStatus", arguments);
			} else {
	...
	this.messageMarkAs = function(id, status) {
		...
		if (_container.length > 0) {
			if ($m.length > 0) {
				...
				return SCS.err.showError(200, "messageMarkAs", arguments);			   
			} else {
	...
	this.messageEnableEdit = function(id) {
		...
		if (_container.length > 0) {
			if ($m.length > 0) {
				$m.addClass("editable");
				return SCS.err.showError(200, "messageEnableEdit", arguments);
			} else {
	...
	this.messageDisableEdit = function(id) {
		...
		if (_container.length > 0) {
			if ($m.length > 0) {
				$m.removeClass("editable");
				return SCS.err.showError(200, "messageDisableEdit", arguments);
			} else {
	...
	this.messageUpdate = function(id, message, time, status) {
		...
		if (_container.length > 0) {
			if ($m.length > 0 && $m.hasClass("message")) {
				...
				return SCS.err.showError(200, "messageUpdate", arguments);
			} else {
	...
	this.messageFollowupStatus = function(id, followup) {
		...
		if (_container.length > 0) {
			if ($m.length > 0 && $m.hasClass("message")) {
				...
				return SCS.err.showError(200, "messageFollowupStatus", arguments);
			} else {
	...
	this.messageStartInlineEdit = function(id) {
		...
		if (_container.length > 0) {
			if ($m.length > 0 && $m.hasClass("message")) {
				...
				return SCS.err.showError(200, "messageStartInlineEdit", arguments);
			} else {
	...
	this.messageEndInlineEdit = function(id) {
		...
		if (_container.length > 0) {
			if ($m.length > 0 && $m.hasClass("message")) {
				...
				return SCS.err.showError(200, "messageEndInlineEdit", arguments);
			} else {
